...
3. Для каждой строки:
   - Пропускаем первую ячейку, если она только цифры (номер строки)
   - Собираем текст: `"cell0 cell1 cell2"` через пробел
   - Пропускаем служебные строки (заголовки, категории)
4. Строки с `SERVICE_ROW_PATTERNS` отбрасываются
...
2. Фильтруем мусор (номера строк, одиночные символы)
3. Собираем разорванные строки: `unit_only` + `qty+unit` → `prev + line`
